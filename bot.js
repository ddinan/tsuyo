...
client.on("guildMemberAdd", member => {
  ...
  const channel = member.guild.channels.find(
    channel => channel.name === guildConf.modLogChannel
  );
...
client.on("guildMemberRemove", member => {
  ...
  const embed = new Discord.RichEmbed()
    .setAuthor("❌ Member left the Guild")
    .setColor(colors.red)
    .setDescription(
      `**Total member count:** \`${member.guild.memberCount}\`\n<@${
        member.user.id
...
client.on("message", async message => {
  ...
  if (message.content.startsWith(`<@492871769485475840>`)) {
    const embed = new Discord.RichEmbed()
      ...
      .addField(
        "Commands",
        `Commands can be found by typing \`${guildConf.prefix}commands\`.`
      )
      .addField(
        "Want to invite me to your Discord?",
        `[Click here to invite me to your server.](https://discordapp.com/oauth2/authorize?client_id=492871769485475840&scope=bot&permissions=1506142455)`
      )
      .addField(
        `Need more assistance?`,
        `[Click here to join the official Tsuyo support server](https://discord.gg/3hbeQgY)`
      );
  ...
  if (message.guild) {
    ...
    const curLevel = Math.floor(
      0.3 * Math.sqrt(client.points.get(key, "points"))
    );
  ...
  if (cmd === "setconf") {
    if (!adminRole)
      return message.reply(`\`${guildConf.adminRole}\` role not found`);

    if (!message.member.roles.has(adminRole.id)) {
      return message.reply(
        `You need the \`${guildConf.adminRole}\` to use this command.`
      );
    }
  ...
  if (cmd === "poll") {
    ...
    if (
      ...
      const embed = new Discord.RichEmbed()
        ...
        .addField(
          input,
          `React with either ${yesEmoji} or ${noEmoji} to vote.`,
          true
        )
        .setTimestamp();
      ...
      if (args.length === 0) {
        message.channel.send(
          `You need to specify the contents of the poll.\nE.g, \`${
            guildConf.prefix
        ...
        message.channel.send(embed).then(message => {
          message.react(yesEmoji).then(() => message.react(noEmoji));
        });
      }
    } else {
        ...
        if (message.mentions.members.size === 0) {
          ...
          kickMember
            ...
            .then(member => {
              ...
              message.channel.send(
                `Successfully kicked member \`${
                  member.user.username
                }\` from the guild.`
              );

              if (channel) {
                // If logs channel exists
  ...
  if (cmd === "daily") {
    ...
    if (daily !== null && timeout - (Date.now() - daily) > 0) {
      ...
      message.channel.send(
        `You already collected your daily reward, you can come back and collect it in **${
          time.hours
  ...
  if (cmd === "give") {
    ...
    message.channel.send(
      `${
        user.tag
      } has received ${pointsToAdd} points and now stands at ${userPoints} points.`
    );
  }
  ...
  if (cmd === "gif" || cmd === "giphy") {
    ...
    const res = await got(
      `https://api.giphy.com/v1/gifs/random?api_key=${api}&tag=${encodeURIComponent(
        args.join(" ")
      )}`,
      { json: true }
    );
    ...
    if (!res || !res.body || !res.body.data)
      return message.channel.send(
        "@Failed to find a gif that matched your query."
      );
  ...
  if (cmd === "rps" || cmd === "spr" || cmd === "psr") {
    ...
    if (input == "rock" || input == "paper" || input == "scissors") {
      ...
      message.channel.send(
        `**INVALID SYNTAX:** ${guildConf.prefix}rps [rock/paper/scissors]`
      );
    }
  }
  ...
  if (cmd === "cmds" || cmd === "commands" || cmd === "c") {
    const embed = new Discord.RichEmbed()
      ...
      .addField(
        `Type ${
          guildConf.prefix
        }commands [category] to view all commands in that category`,
    ...
    const admin = new Discord.RichEmbed()
      ...
      .addField(
        ...
        `\`${
          guildConf.prefix
        }showconf\` - Shows the settings for the guild.\n\n\`${
          guildConf.prefix
        }setconf [setting] [input]\` - Sets [setting] to [input] in the guild you are in.`
      );
    ...
    const eco = new Discord.RichEmbed()
      ...
      .addField(
        "💰 Economy Commands",
        `\`${guildConf.prefix}bal\` - Shows your balance.\n\n\`${
          guildConf.prefix
        }daily\` - Claims your daily bonus.\n\n\`${
          guildConf.prefix
        }leaderboard\` - Shows the top 10 users in the Discord with the most XP.\n\n\`${
          guildConf.prefix
        }Store\` - Shows buyable items.`
      );
    ...
    const fun = new Discord.RichEmbed()
      ...
      .addField(
        ...
        `\`${
          guildConf.prefix
        }8ball [question]\` - Responds with "yes, no or maybe" or if containing "who", it will randomly select a user in the Discord.\n\n\`${
          guildConf.prefix
        }coin\` - Flips a coin. Great for disputes.\n\n\`${
          guildConf.prefix
        }gif [query]\` - Searches for a gif based on your query.\n\n\`${
          guildConf.prefix
    ...
    const info = new Discord.RichEmbed()
      ...
      .addField(
        ...
        `\`${
          guildConf.prefix
        }avatar <@user>\` - Shows the avatar of the specified user. If <@user> isn't specified, it will show your avatar instead.\n\n\`${
          guildConf.prefix
        }bot\` - Shows info about the bot.\n\n\`${
          guildConf.prefix
        }commands [category]\` - Shows all commands in [category].\n\n\`${
          guildConf.prefix
        }help\` - Shows information and a link to the official Tsuyo Discord.\n\n\`${
          guildConf.prefix
        }info\` -  Shows information about the specified user. If <@user> isn't specified, it will show your information instead.\n\n\`${
          guildConf.prefix
        }ping\` - Shows your ping in milliseconds.\n\n\`${
          guildConf.prefix
        }remind [time] [message]\` - Messages you [message] after [time]. Use 's' for seconds, 'm' for minutes, 'h' for hours and 'd' for days. If a measurement of time is not specified, the time will be in seconds.\n\n\`${
          guildConf.prefix
        }server\` - Shows information about the server.\n\n\`${
          guildConf.prefix
        }vote\` - Shows voting links.`
      );
    ...
    const mod = new Discord.RichEmbed()
      ...
      .addField(
        ...
        `\`${
          guildConf.prefix
        }ban [user]\` - Bans [@user] from the guild.\n\n\`${
          guildConf.prefix
        }kick [@user]\` - Kicks [@user] from the guild.\n\n\`${
          guildConf.prefix
        }mute [@user]\` - Prevents [@user] from talking.\n\n\`${
          guildConf.prefix
        }poll [question]\` - Creates a yes or no poll for [input].\n\n\`${
          guildConf.prefix
        }prune [1-99]\` - Deletes specified amount of messages.\n\n\`${
          guildConf.prefix
        }react [message ID] :emoji:\` - Reacts to [message ID] with :emoji:.\n\n\`${
          guildConf.prefix
  ...
  if (cmd === "donate") {
    const embed = new Discord.RichEmbed()
      ...
      .addField(
        `Want exclusive donator perks and more bot features?`,
        `[Click here to donate.](https://www.patreon.com/TsuyoBot)`
      )
      .addField(
        `Donator Perks`,
  ...
  if (cmd === "remind" || cmd === "remindme") {
    ...
    if (reminder) {
      ...
      const fail = new Discord.RichEmbed()
        ...
        .setTitle("**FAIL:**")
        .setDescription(
          `I couldn't send you a DM. Please check to see if you have direct messaging enabled.`
        )
        .setTimestamp();
  ...
  if (cmd === "server" || cmd === "serverinfo") {
    const embed = new Discord.RichEmbed()
      ...
      .addField(
        "Created",
        `By **${message.guild.owner}** on ${message.guild.createdAt}`,
        true
      )
      .addField(
        "AFK",
        `**Channel:** ${message.guild.afkChannel}\n **Timeout:** ${
    ...
    const embed2 = new Discord.RichEmbed()
      ...
      .addField(
        "Created",
        `By **${message.guild.owner.user}** on ${message.guild.createdAt}`,
        true
      )
      .addField(
        "AFK",
  ...
  if (cmd === "vote") {
    const embed = new Discord.RichEmbed()
      ...
      .addField(
        `Want to help out?`,
        `[Click here to vote.](https://discordbots.org/bot/492871769485475840/vote)`
      )
