  ...
  client.Embed = class {
    constructor(type, settings) {
      ...
      if (this.settings.description)
        this.embed.setDescription(this.settings.description);

      if (this.settings.fields) {
        this.settings.fields.forEach((field) => {
          if (field == "blank") this.embed.addBlankField();
          else
            this.embed.addField(field.title, field.text, field.inline || false);
        });
      }
      ...
      if (this.settings.thumbnail)
        this.embed.setThumbnail(this.settings.thumbnail);

      if (this.settings.author)
        this.embed.setAuthor(
          this.settings.author.name,
          this.settings.author.icon || null,
          this.settings.author.url || null
        );
